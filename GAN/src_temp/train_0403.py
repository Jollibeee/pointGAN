...
for epoch in tqdm(range(opt.nepoch)):
    ...
    while i < len(dataloader):
        ...
        if diter % 10 ==0:
            summary.add_scalar('loss/lossD', lossD.item(), diter)
            summary.add_scalar('loss/lossG', lossG.item(), diter)
            summary.add_scalar('loss/loss', {"lossD": lossD.item(), "lossG": lossG.item()}, diter)
    ...
    fake_points_np = fake_points.transpose(2, 1)
    sio.savemat('%s/Gen_points_%d.mat' %(renf, epoch), fake_points_np)
