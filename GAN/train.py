...
parser.add_argument('--outf', type=str, default='pointGAN',  help='output folder')
# parser.add_argument('--model', type=str, default = 'wgan/model',  help='model path')
parser.add_argument('--model', type=str, default = '',  help='model path')
parser.add_argument('--start_epoch', type=int, default = '0',  help='model path')
...
if opt.model != '':
    print("load model")
    path = '%s/model_%d.pth' %(opt.model, opt.start_epoch)
...
for epoch in tqdm(range(opt.nepoch)):
    ...
    # while i < 5:
        for diter in range(opt.Diters):
            ...
            ## Train with real
            pred_real, _ = dis(points)
            loss_real = torch.mean(pred_real)

            noise = Variable(torch.randn(bs, opt.feat_dim)).cuda()  # batch, 1, z_dim(128)
            # print("fake noise = ", fake_noise.size())
            fake = gen(noise)       # batch, 3-dim, num_points
            # fake_np = fake.transpose(2,1).data.cpu().numpy()    # batch, num_points, 3-dim

            pred_fake, _ = dis(fake)
        ...
        noise = Variable(torch.randn(bs, opt.feat_dim)).cuda()
        fake = gen(noise)

        pred_fake, _ = dis(fake)
        lossG = torch.mean(pred_fake)
        # lossG.backward(mone)
        lossG.backward(one)
        # lossG.backward()
        ...
        summary.add_histogram('synthetic_logit', pred_fake)
        summary.add_histogram('real_logit', pred_real)
        # summary.close()
