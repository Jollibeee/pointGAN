...
def parseRenderCmds(PATH_xml, PATH_png, skipExist, mitsuba_arg):
    ...
    renderCmds = []
    # pbar = tqdm(range(1, len(xmlfiles)))
    for xmlfile in xmlfiles:
        ...
        if isfile(outfullfile) is True and skipExist is True:
            ...
            continue
        renderCmds.append("mitsuba -q -x " + mitsuba_arg + ' ' + xmlfullfile + " -o " + outfullfile)
        # renderCmds.append("mitsuba -x " + mitsuba_arg + ' ' + xmlfullfile + " -o " + outfullfile)
        # pbar.update(1)
    # pbar.close()
    return renderCmds
