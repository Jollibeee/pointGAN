...
from src.plyfile import PlyElement, PlyData
...
def unpickle_data(file_name):
    ...
    for _ in range(size):
        ...
        #yield cPickle.load(inFile, encoding='latin1')  ###### yield cPickle.load(inFile) #https://bladewalker.tistory.com/681  ## 수정 필요
    inFile.close()
...
def files_in_subdirs(top_dir, search_pattern):
    for path, dirs, files in os.walk(top_dir):
        for file in files:
            if file.endswith(search_pattern):
                full_name = osp.join(path, file)
                yield full_name
...
def load_all_point_clouds_under_folder(top_dir, n_threads=20, file_ending='.ply', verbose=False):
    ...
    file_names = [f for f in os.listdir(top_dir) if osp.isfile(osp.join(top_dir, f))]
    file_names = [x for x in file_names if x.find(file_ending) != -1]
    file_names = [osp.join(top_dir, t) for t in file_names]
