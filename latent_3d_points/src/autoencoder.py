...
from src.in_out import create_dir, pickle_data, unpickle_data
from src.general_utils import apply_augmentations, iterate_in_chunks
from src.neural_net import Neural_Net, MODEL_SAVER_ID
...
class LDGCNN_AutoEncoder(Neural_Net):
    ...
    def __init__(self, name, graph, configuration):
        ...
        out_shape = [self.batch_size] + self.n_output
    ...
    def partial_fit(self, X, GT=None):
        ...
        try:
            if GT is not None:
                _, loss, recon = self.sess.run((self.train_step, self.loss, self.x_reconstr),
    ...
    def train(self, train_data, configuration, log_file=None, held_out_data=None):
        ...
        for _ in range(c.training_epochs):
            ...
            if epoch % c.loss_display_step == 0:
                # print("Epoch:", '%04d' % (epoch), 'training time = ', "{:%2d:%2d:%2d}".format(hh, mm, ss), "loss=", "{:.9f}".format(loss))
                print("Epoch: %04d \t training time = %2d:%2d:%2d \t loss = %.9f" % (epoch, hh, mm, ss, loss))
